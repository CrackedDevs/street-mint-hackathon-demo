...
import { getGalleryInformationByTokenAddresses, QuantityType } from "@/lib/supabaseClient"
...
export const GalleryGrid = () => {
  ...
  async function fetchNFTs() {
    ...
    try {
      ...
      if (data.result) {
        ...
        const galleryInformation = await getGalleryInformationByTokenAddresses(nfts);
  ...
  return (
    <div className="container mx-auto text-gray-800">
        ...
        <h2 className="text-5xl font-bold text-black mt-10 z-30">Your Collectibles</h2>
        </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 mt-6 mx-4">
        {nfts.map((nft) => (
          <Card key={nft.mintAddress} className="overflow-hidden bg-white z-30 flex flex-col">
          ...
          <CardContent className="p-4 flex-grow">
            <CardTitle className="text-lg font-bold mb-2">{nft.collectibleName}</CardTitle>
          ...
          <CardFooter className="p-4 pt-0">
            <Button 
              className="w-full" 
              variant="outline"
              onClick={() => window.open(SolanaFMService.getAddress(nft.mintAddress), '_blank')}
            >
